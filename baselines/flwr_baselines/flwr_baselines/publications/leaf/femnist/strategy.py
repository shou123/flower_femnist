...
class include_or_exclude_clients_random_with_overlap_probability(FedAvg):
    """Configurable FedAvg strategy implementation with random client selection."""
    ...
    def configure_fit(
        ...
        if self.include_clients_0_and_1:
            ...
            # )
            # remaining_clients = [client for client in all_clients if client.cid not in inclusive_client_ids]
            for cid in inclusive_client_ids:
                client = client_manager.clients.get(cid)
                if client and random.random() < self.overlap_probability:
                    selected_client_proxies.append(client)
            remaining_clients = [
                client for client in all_clients
                if client.cid not in inclusive_client_ids or random.random() >= self.overlap_probability
            ]
        else:
            remaining_clients = [client for client in all_clients if client.cid not in inclusive_client_ids]
