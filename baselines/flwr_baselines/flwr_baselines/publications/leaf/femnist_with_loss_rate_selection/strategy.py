...
class FedAvgSameClients(FedAvg):
    ...
    # ) -> Optional[Tuple[float, Dict[str, Scalar]]]:
    #     """Aggregate evaluation results and update client losses."""
    #     aggregated_result = super().aggregate_evaluate(server_round, results, failures)
        
    #     # Update client losses
    #     for client, metrics in results:
    #         if "loss" in metrics:
    #             self.client_losses[client] = metrics["loss"]
    ...
    def aggregate_fit(
        ...
        for client_proxy, fit_res in results:
            ...
            if "train_loss" in fit_res.metrics:
                self.client_losses[client_id] = fit_res.metrics["train_loss"]
